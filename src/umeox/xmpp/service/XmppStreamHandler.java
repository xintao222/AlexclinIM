...
public class XmppStreamHandler {
	...
	protected final Collection<AckReceivedListener> ackListeners = new CopyOnWriteArraySet<AckReceivedListener>();
	...
	public void quickShutdown() {
		if (isResumePossible()) {
			...
			// We will not necessarily get any notification from a
			// quickShutdown, so adjust our state here.
			closeOnError();
		} else {
			// XXX: shutdown() would cause the connectionClosed listener to be
			// called!
			mConnection.quickShutdown();
			close();
		}
	...
	private void sendEnablePacket() {
		...
		if (sessionId != null) {
			...
			StreamHandlingPacket resumePacket = new StreamHandlingPacket(
					"resume", URN_SM_2);
			resumePacket.addAttribute("h",
					String.valueOf(previousIncomingStanzaCount));
			...
			StreamHandlingPacket enablePacket = new StreamHandlingPacket(
					"enable", URN_SM_2);
	...
	private void startListening() {
		mConnection.addConnectionListener(new ConnectionListener() {
			public void reconnectionSuccessful() {
				synchronized (XmppStreamHandler.this) {
					if (isResumePossible() && !isSmAvailable) {
						Log.d(TAG,
								"reconnected, waiting for SM packet to arrive...");
						try {
							XmppStreamHandler.this.wait(30000); // HACK: wait
																// for SM packet
																// to arrive
						} catch (InterruptedException e) {
						}
			...
			public void connectionClosedOnError(Exception e) {
				Log.d(TAG,
						"connectionClosedOnError: " + e.getLocalizedMessage());
			...
			public void connectionClosed() {
				...
				// the connectionClosed callback is called from
				// PacketReader.shutdown()
				// - this can also happen when a connection attempt failed, so
				// we can not
				// assume a proper termination. Instead, try to recover SM if
				// possible.
				closeOnError();
			}
		});
		...
		mConnection.addPacketSendingListener(new PacketListener() {
			public void processPacket(Packet packet) {
				...
				if (isOutgoingSmEnabled && !outgoingQueue.contains(packet)) {
					...
					if (debugStanzas)
						Log.d(TAG, "adding " + outgoingStanzaCount + " : "
								+ packet.toXML());
					//
					// Don't let the queue grow beyond max size. Request acks
					// and drop old packets
		...
		mConnection.addPacketListener(new PacketListener() {
			public void processPacket(Packet packet) {
				if (isSmEnabled && isStanza(packet)) {
					incomingStanzaCount++;
					if (debugStanzas)
						Log.d(TAG, "recv " + incomingStanzaCount + " : "
								+ packet.toXML());
				} else {
					if (debugStanzas)
				...
				if (packet instanceof StreamHandlingPacket) {
					...
					if ("sm".equals(name)) {
						...
						StreamHandlingPacket ackPacket = new StreamHandlingPacket(
								"a", URN_SM_2);
						ackPacket.addAttribute("h",
								String.valueOf(incomingStanzaCount));
						mConnection.sendPacket(ackPacket);
					} else if ("a".equals(name)) {
						long ackCount = Long
								.valueOf(shPacket.getAttribute("h"));
						removeOutgoingAcked(ackCount);
						Log.d(TAG, outgoingQueue.size()
								+ " in outgoing queue after ack");
						...
						long resumeStanzaCount = Long.valueOf(shPacket
								.getAttribute("h"));
						// Removed acked packets
						removeOutgoingAcked(resumeStanzaCount);
						Log.d(TAG, outgoingQueue.size()
								+ " in outgoing queue after resume");
						...
						// Enable only after resend, so that the interceptor
						// does not
						...
						mConnection.causeException(new Exception(
								"XEP-0198 stream resumption failed"));
	...
	private void removeOutgoingAcked(long ackCount) {
		if (ackCount > outgoingStanzaCount) {
			Log.e(TAG, "got ack of " + ackCount + " but only sent "
					+ outgoingStanzaCount);
			// Reset the outgoing count here in a feeble attempt to re-sync. All
			// bets
			// are off.
	...
	private static void addSimplePacketExtension(final String name,
			final String namespace) {
		ProviderManager.getInstance().addExtensionProvider(name, namespace,
				new PacketExtensionProvider() {
					public PacketExtension parseExtension(XmlPullParser parser)
							throws Exception {
						StreamHandlingPacket packet = new StreamHandlingPacket(
								name, namespace);
		...
		public String toXML() {
			...
			for (String key : attributes.keySet()) {
				buf.append(" ").append(key).append("=\"")
						.append(StringUtils.escapeForXML(attributes.get(key)))
						.append("\"");
			}
			buf.append("/>");
			return buf.toString();
	...
	/**
	 * Returns true if the packet is a Stanza as defined in RFC-6121 - a
	...
	public static interface AckReceivedListener {
		 ...
		 * 
		 * @param handled
		 *            number of stanzas handled by the other side
		 * @param total
		 *            number of stanzas sent to other side
		 */
	...
	public static class ExtXMPPConnection extends XMPPConnection {
		...
		public void causeException(Exception e) {
			...
			try {
				java.lang.reflect.Method nce = XMPPConnection.class
						.getDeclaredMethod("notifyConnectionError",
